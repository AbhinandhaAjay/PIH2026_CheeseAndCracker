    ...
    <div className="px-4 py-4 sm:px-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <button
          ...
          className="inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-red-500 hover:bg-red-700"
        >
...
const PoliceDashboard = () => {
  ...
  const handleAccept = async (id) => {
    try {
      ...
      if (!res.ok) return;
      setIncidents((prev) => prev.map((i) => i.id === id ? { ...i, police_status: "accepted" } : i));
    } catch (err) {
  ...
  const pending = incidents.filter((i) => i.police_status === "pending");
  const accepted = incidents.filter((i) => i.police_status === "accepted");
  const rejected = incidents.filter((i) => i.police_status === "rejected");
