...
def send_to_django(address, description, severity, severity_score, image_path=None):
    data = {
        'address': address,
    ...
    try:
        response = requests.post(
            f'{DJANGO_API_URL}/api/accidents/create/',
            data=data,
            files=files if files else None  # Only include files if image exists
        )
...
def accident(video_path,address):
    ...
    prompt = f"""
    You are an intelligent accident analysis assistant. Based on the accident report below and the attached images, generate a clear and concise summary of how the accident occurred. Use the visual evidence to support the explanation, and describe the scenario as if explaining it to a non-technical person. Each line should have a maximum of 13 words.
    ...
    *   A Orange car.
    ...
    A severe accident occurred involving the vehicles. The orange car was travelling at 7 km/h, 
    and it collided with the white car travelling at 6 km/h.

    The impact caused the orange car to displace approximately 4.47 meters and the white car 
    to displace about 14.93 meters.

    Severity:

    The accident is a "SevereAccident" with a severity score of 4, indicating a significant 
    level of damage and potential injuries.

    Keep the tone factual, easy to understand, and suitable for informing a concerned viewer 
    or traffic authority.
    don't mention based on the given infos or images in the starting.
    """
    ...
    # Send data to Django
    image_path = images[0]  # Choose the best/first image to send
    send_to_django(address, description, severity, severity_score, image_path)
